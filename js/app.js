(function() {
  ...
  app.controller('NavigationController', ['$scope', '$http',
    ...
    $rootScope.page_title = "Listen"; // page title
    $scope.window_url_stack = []; // 
    $scope.current_tag = 2; // 初始页面 通过current_tag来控制那个页面显示 结合ng-show
    ...
    $scope.showWindow = function(url){
      ...
      $scope.is_window_hidden = 0; // 展示歌单窗口
      $scope.resetWindow();
  ...
  app.controller('PlayListController', ['$http','$scope', '$timeout',
                                        'angularPlayer','loWeb',
                                        function($http, $scope, $timeout, angularPlayer, loWeb){
    $scope.result = []; // result 直接用于数据渲染
    $scope.tab = 0;  //  歌曲源选择 默认网易云
    $scope.loading = true 
